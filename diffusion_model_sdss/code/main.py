...
def main():
    ...
    parser.add_argument('--num_epochs', type=int, default=1000, help="last  epoch")
    parser.add_argument('--start_epoch', type=int, default=0, help="first epoch")
    parser.add_argument('--lr_freq', type=int, default=100)
    parser.add_argument(  #JEC
        "--resume_model",
        action='store_true',   
        default=False,
        help="if True load model params for continuing training",
    )
    parser.add_argument( #JEC
        "--resume_optimizer",
        action='store_true',
        default=False,
        help="if True load optimizer params for continuing training",
    )
    ...
    parser.add_argument('--debug', action='store_true', default=False)  # JEC action sture
    ...
    ### load raw data
    train_set = torch.load(args.data_path + '_train_no_repeats_64x64.pt')
    ...
    if args.resume_model:
        if args.coarse:
            fname = args.dir_name  + '/model.pt'
        else:
            fname = args.dir_name  + '/model_scale'+str(args.SLURM_ARRAY_TASK_ID)+'.pt'
        print("resume saved model:",fname)

        model.load_state_dict(torch.load(fname))
        if args.start_epoch==0: # if not the user set it already on command line
            info_dict = torch.load(args.dir_name + '/optim_saved.pt')
            args.start_epoch = info_dict['state'][0]['step']+1
    ...
    if args.resume_optimizer:
        print("resume saved optimizer")
        optimizer.load_state_dict(torch.load(args.dir_name + '/optim_saved.pt'))
