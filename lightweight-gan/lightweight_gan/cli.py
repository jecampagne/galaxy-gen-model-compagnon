...
def run_training(rank, world_size, model_args, data,
                 ndata, # JEC
                 load_from, new, num_train_steps, name, seed, use_aim, aim_repo, aim_run_hash):
    is_main = rank == 0
    is_ddp = world_size > 1
    ...
    print("JEC cli: ndata option")
    model.set_data_src(data, ndata=ndata) # add ndata JEC 19Sept24

    print("JEC time start")
    ...
    print("JEC duration (minutes) ",(time.time() - start_time)/60.)
...
def train_from_folder(
    ...
    if world_size == 1 or not multi_gpus:
        run_training(0, 1, model_args, data,
                     ndata, #JEC
                     load_from, new, num_train_steps, name, seed, use_aim, aim_repo, aim_run_hash)
        return

    mp.spawn(run_training,
        args=(world_size, model_args, data,
              ndata, #JEC
              load_from, new, num_train_steps, name, seed, use_aim, aim_repo, aim_run_hash,),
        nprocs=world_size,
        join=True)
