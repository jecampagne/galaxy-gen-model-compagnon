...
import random                #JEC  so random() is modified in random.random() in the code
import math
...
from torch.utils.data import Dataset, DataLoader, Subset  # add Subset JEC 19Sept24
...
#not valid at IDRIS/Jean-Zay super-computer from adabelief_pytorch import AdaBelief
...
####test no aug            transforms.RandomHorizontalFlip(), # JEC
####test no aug            transforms.RandomVerticalFlip(),   # JEC
####test no aug            RandomApply(aug_prob, transforms.RandomResizedCrop(image_size, scale=(0.5, 1.0), ratio=(0.98, 1.02)), transforms.CenterCrop(image_size)),
            transforms.ToTensor(),
...
class LightweightGAN(nn.Module):
    def __init__(
        ...
        *,
        ...
        n_param_G =  sum(p.numel() for p in self.G.parameters() if p.requires_grad)
        n_param_D =  sum(p.numel() for p in self.D.parameters() if p.requires_grad)
        
        print('JEC total number of parameters is (million) ' , (n_param_G + n_param_D)/1e6)


        if optimizer == "adam":
            self.G_opt = Adam(self.G.parameters(), lr = lr, betas=(0.5, 0.9))
            self.D_opt = Adam(self.D.parameters(), lr = lr * ttur_mult, betas=(0.5, 0.9))
#not at IDRIS        elif optimizer == "adabelief":
...
class Trainer():
    ...
    def set_data_src(self, folder, ndata):  # add ndata JEC 19Sept24
        print('Dbg JEC ndata=',ndata)
        num_workers = default(self.num_workers, math.ceil(NUM_CORES / self.world_size))
        dataset = ImageDataset(folder, self.image_size, transparent = self.transparent, greyscale = self.greyscale, aug_prob = self.dataset_aug_prob)
        self.dataset = Subset(dataset, torch.arange(ndata)) # JEC
        print('Dbg JEC len dataset: ',len(self.dataset))
    ...
    def generate(self, num=0, num_image_tiles=4, checkpoint=None, types=['default', 'ema'],
        ...
        if 'default' in types:
            ...
            if num_image_tiles > 1e5:
                print(f"Warning you've asked for  {num_image_tiles} => the file would be truncated at 1e5 images")
                num_image_tiles =1e5
            all_imgs = np.zeros( (num_image_tiles,64,64), dtype=np.uint8)
            for i in tqdm(range(num_image_tiles), desc='Saving generated default images'):
                ...
                im = image_to_pil(generated_image[0]) # JEC 
                im = torchvision.transforms.functional.resize(im,64)   # reduce to 64x64 JEC
                im = convert_image_to('L',im) #
                arr = np.array(im)  # 64x64 uint8
                all_imgs[i] = arr

            #save
            path = str(self.results_dir / dir_name / f'gan-samples_{str(all_imgs.shape[0])}.npy')
            np.save(path,all_imgs)
    ...
    def get_checkpoints(self):
        ...
        print("JEC (Dbg): lw_gan:get_checkpoints:saved_nums =",saved_nums)
        return saved_nums
